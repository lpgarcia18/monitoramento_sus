					...
					column(12,
						...
						column(4,
						       ...
						       selectInput(inputId = ns("faixa_etaria_select"),
						       	    label = "Selecione a Faixa Etaria",
...
producao_SERV <- function(id, banco){
	...
	banco_preparado <- reactive({
		 ...
		 producao_select  <- banco  %>%
			subset(banco$tipo_atendimento == input$tipo_atendimento_select &
			       	banco$tipo_unidade == input$tipo_unidade_select &
			       	banco$profissao == input$profissao_select &
			       	banco$sexo == input$sexo_select &
			       	banco$faixa_etaria == input$faixa_etaria_select  &
			       	banco$mes_atendimento >=  startdate_sh() &
			       	banco$mes_atendimento <=  enddate_sh()
			)
		producao_select
...
# Gráfico e tabela análise --------------------------------------------------------
	output$producao_graf <- renderPlotly({
		graf <- ggplot(banco_preparado(), aes(as.Date(banco_preparado()$mes_atendimento), as.numeric(banco_preparado()$quantidade)))+
			geom_line()+
			xlab("Ano") +
			ylab("")+
